...
class ScaleTest(object):
    ...
    def get_vm_active_time(self, initial, expected):
        ...
        if current - initial < expected:
            logger.info('Not all VMs are active, actual: %s, expected: %s',(
    ...
    def setUp(self):
        ...
        logger.info('Time to create all projects %s', timediff)
        ...
        if self._args.verify:
            (success, timediff) = create_n_process(self.verify,
                                                   ...
                                                   kwargs_list)
            logger.info( 'Time to verify all projects %s', timediff, 'Success Percentage: %s', success)
    ...
    def post_create(self):
        ...
        if self._args.n_vms or self._args.n_svc_chains:
            ...
            logger.info( "Took %s secs to have all vms in active state" %(
            ...
            if current_port_count - self.initial_port_count != exp_ports:
                logger.info( 'Port count mismatch, current:%s, expected:%s'%(
    ...
    def cleanup(self):
        ...
        (success, timediff) = create_n_process(self.delete, 
                                               ...
                                               kwargs_list,
                                               self._args.timeout)
        logger.info( 'Time to delete all projects %s', timediff)
...
class PerprojectWrapper(object):
    ...
    def setUp(self):
        ...
        (success, timediff) = create_n_process(self.start_create,
                                               ...
                                               self._args.timeout,
                                               callback=self.read_from_queue)
        logger.info( 'Time to create objects for project %s is %s. Success %% %s' %(
    ...
    def cleanup(self):
        ...
        (success, timediff) = create_n_process(self.start_cleanup, 
                                               ...
                                               kwargs_list,
                                               self._args.timeout)
        logger.info( 'Time to delete objects for project %s is %s. Success %% %s' %(
               self._args.project, timediff, success))
        self.post_cleanup()
...
class VNC(Openstack):
    def __init__(self, auth_url, username, password, project, ip, port, auth_host, auth_token=None):
        super(VNC, self).__init__(auth_url, username, password, project, auth_token)
...
def parse_cli(args):
    ...
    if pargs.n_projects > 1 and pargs.project:
        pargs.project = None
        logger.info( 'Overriding --project as --n_projects is set')
        time.sleep(1)

    if pargs.n_vms and pargs.n_ports and (pargs.n_vms != pargs.n_ports):
        pargs.n_ports = pargs.n_vms
        logger.info( 'Setting n_ports to be same as n_vms')
        time.sleep(1)
...
def create_n_process(target, n_process, kwargs_list, timeout=None, callback=None):
    ...
    if debug is True:
        logger.info( 'Time at start %s', str(start_time))
    ...
    if debug is True:
        logger.info( 'Time at End %s', str(end_time))
